...
This runs the scanner against the current Python process, changes a test integer in memory, and confirms the write worked.

## Current limitations

- It assumes the target value is a 32-bit signed integer.
- It is not a static one-click trainer yet, so you need to scan and refine after each fresh game launch.
- Unlock flags, staff data, facility stats, and item states still need deeper reverse engineering before they can be exposed as dedicated buttons.
